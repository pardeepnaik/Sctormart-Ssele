 ...
 * @author Gezim Sejdiu
 */
class RDFStatistics(a:RDD[Triple],spark:SparkSession) extends Serializable {
 ...
 // @transient val spark: SparkSession = SparkSession.builder().getOrCreate()
   ...
   */
  def run(a:RDD[Triple]) = {
    Used_Classes(a, spark).Voidify()
      .union(DistinctEntities(a, spark).Voidify)
      .union(DistinctSubjects(a, spark).Voidify)
      .union(DistinctObjects(a, spark).Voidify)
      .union(PropertyUsage(a, spark).Voidify)
      .union(SPO_Vocabularies(a, spark).Voidify)
  }
   ...
   *
   * @param triples RDD of triples
   * @return number of triples that are referencing literals to objects.
   */
  ...
  def BlanksAsSubject(triples: RDD[Triple]) =
    triples.filter(_.getSubject.isBlank()).count()
  ...
  def TypedSubjects(triples: RDD[Triple]) =
    triples.filter(triple => triple.getPredicate.toString().contains("type")).map(_.getSubject).count()
  ...
  def LabeledSubjects(triples: RDD[Triple]) =
    triples.filter(triple => triple.getPredicate.toString().contains("label")).map(_.getSubject).count()
  ...
  def DistinctPredicates(triples: RDD[Triple]) = {
    def Filter() = triples
    def Action() = Filter().map(_.getPredicate).distinct().count()
    Action()
  }
...
object RDFStatistics {
  def apply(rdfgraph:RDD[Triple],spark:SparkSession) = new RDFStatistics(rdfgraph,spark) //.run()
}
